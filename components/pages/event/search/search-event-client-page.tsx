...
const swrFetcher = async ([, name, startDate, minPrice, maxPrice, type]: [
  string,
  string,
  string,
  number,
...
export function SearchEventClient({ lang, dictionary }: Props) {
    ...
    <div className="container py-8">
      <div className="mb-6 space-y-4">
        <div>
          <Label htmlFor="name" className="block font-semibold mb-2">
            {dictionary.search?.eventName || "Nome do Evento"}
          </Label>
          <Input
            id="name"
            placeholder={
              dictionary.search?.eventNamePlaceholder ||
        ...
        <div className="flex flex-col md:flex-row items-start gap-4">
          <div className="flex flex-col">
            ...
            {selectedDate && (
              ...
              >
                {dictionary.search?.clearDate || "Limpar data"}
              </button>
            )}
          </div>
          ...
          <div>
            <Select value={searchType} onValueChange={setSearchType}>
              <SelectTrigger className="bg-neutral-50 w-[200px]">
                <SelectValue
                  placeholder={
                    dictionary.search?.selectType || "Selecione um tipo"
                  }
                />
              </SelectTrigger>
              <SelectContent>
      ...
      {isLoading ? (
        ...
        <div className="text-center text-red-500">
          {dictionary.search?.error || "Ocorreu um erro ao buscar eventos."}
        </div>
        ...
        <p className="text-center text-gray-500">
          {dictionary.search?.noEvents || "Nenhum evento encontrado"}
        </p>
      )}
    </div>
  );
